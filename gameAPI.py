...
class Game:
    ...
    def get_my_units(self):
        if self.playerId == 1:
            return convert_json_to_units(self.game['p1Units'])
        else:
            return convert_json_to_units(self.game['p2Units'])
    ...
    # Returns the unit at the given position, if there is one. Takes a position tuple (x,y)  v
    ...
    def path_to(self, start_position, end_position, tiles_to_avoid = []):
        ...
        while len(q) > 0:
            ...
            if not ((left[0] < 0) or (left in tiles_to_avoid) or (self.getTile(left).type != 'BLANK')):
            ...
            if not ((up[1] >= len(self.game['map']['tiles'])) or (up in tiles_to_avoid) or (self.get_tile(up).type != 'BLANK')):
